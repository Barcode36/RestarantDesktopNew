...
public class ViewSubCatagoryController implements Initializable {
    ...
    public void sendEditCategoryDetails()
    {
        ...
        try {
            jsonObject.put("sub_cat_name", txtFieldName.getText());
            jsonObject.put("sub_cat_tag", txtFieldTagLine.getText());
            jsonObject.put("sub_cat_image", outputImage);
            jsonObject.put("sub_cat_id",txtFieldId.getText());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        Call<RequestAndResponseModel> call = retrofitClient.editSubCategory(jsonObject);
...
//                                RequestAndResponseModel.list currentPerson = (RequestAndResponseModel.list) deleteButton.this.getTableView().getItems().get(ButtonCell.this.getIndex());
                    ...
                    editButton.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
                        ...
                        public void handle(MouseEvent event) {
                            ...
                            Image image = new Image(Constants.SUB_CATEGORY_BASE_URL + person.getImage());
                            imgEditImage.setImage(image);
...
//        addData();
    ...
    private void setViewItem(RequestAndResponseModel.list item) {
        ...
        Image image = new Image(Constants.SUB_CATEGORY_BASE_URL + item.getImage());
        System.out.println(Constants.SUB_CATEGORY_BASE_URL + item.getImage());
    ...
    private void getData() {
        ...
        Call<RequestAndResponseModel> call = retrofitClient.viewSubCategory(jsonObject);
        call.enqueue(new Callback<RequestAndResponseModel>() {
            @Override
            public void onResponse(Call<RequestAndResponseModel> call, Response<RequestAndResponseModel> response) {
                if (response.isSuccessful()) {
    ...
    private void deleteItem(RequestAndResponseModel.list itemId) {
        ...
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("sub_cat_id", itemId.getId());

        } catch (JSONException e) {
            e.printStackTrace();
        }

        Call<RequestAndResponseModel> call = retrofitClient.deleteSubCategory(jsonObject);
        call.enqueue(new Callback<RequestAndResponseModel>() {
            @Override
            public void onResponse(Call<RequestAndResponseModel> call, Response<RequestAndResponseModel> response) {
                if (response.isSuccessful()) {
